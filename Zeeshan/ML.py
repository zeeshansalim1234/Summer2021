...
plt.rc('axes', labelsize=5.5)    # fontsize of the x and y labels
labels=['1 Police Services','2 Sidewalks & Pathways','3 Investigate Further ','4 Other or N/A','5 Streets','6 Public Transit','7 Parks and Open Spaces','Negative']
...
def SVM(tweets,corresponding_labels,status):
    ...
    #print(f1_score(y_test,y_prediction,average=None,labels=['1 Police Services','2 Sidewalks & Pathways','3 Investigate Further ','4 Other or N/A','Negative']))
    print("\nLinear SVM Accuracy (" + status + " data) : " + str(metrics.accuracy_score(y_test, y_prediction)))
    print("Linear SVM F1 Score (" + status + " data) : " + str(metrics.f1_score(y_test, y_prediction, average='weighted', labels=np.unique(y_prediction))))
...
def generate_ROC_curve(y_test,y_prediction):
    ...
    logistic_fpr, logistic_tpr, threshold = roc_curve(y_test,y_prediction)
...
def main():
    ...
    counter=0
    for i in range(0,len(corresponding_labels)):
        if(corresponding_labels[i]=='4 Other or N/A'or corresponding_labels[i]=='Negative'):
            counter+=1

    print("\n\nNumber of tweets in Category 4 and Negative : "+str(counter/len(corresponding_labels)*100)+"%")

    pre_processed_tweets=pre_process_tweets(tweets)

    bernoulli_model(tweets,corresponding_labels,"raw")                         #raw dataset
    bernoulli_model(pre_processed_tweets,corresponding_labels,"pre-processed") #pre-processed dataset

    logistic_regression(tweets,corresponding_labels,"raw")
    logistic_regression(pre_processed_tweets, corresponding_labels, "pre-processed")

    SVM(tweets, corresponding_labels, "raw")
    SVM(pre_processed_tweets, corresponding_labels, "pre-processed")
