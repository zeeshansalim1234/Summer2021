...
def bernoulli_model(df_train,tweets,corresponding_labels,predict_tweets,status):
    ...
    for i in range(0,len(tweets)):
        ...
        tokenized.append(tokenizer.encode(tweets[i],add_special_tokens=True,padding='longest',max_length=500))
    """
...
def main():
    ...
    train_tweets = df_train[0].to_list()
    train_corresponding_labels = df_train[1].to_list()
    ...
    prediction=bernoulli_model(df_train,train_pre_processed_tweets,train_corresponding_labels,predict_pre_processed_tweets,"pre-processed") #pre-processed dataset
