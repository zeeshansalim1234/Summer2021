...
def generate_charity_data(result,description,organization):


    for i in range(0,len(description)):
        result.append(organization[i]+" "+description[i])
...
def main():
    ...
    features = ['Description', 'OrganizationName']
    df=extract_database()
    df['index']=df.index
    ...
    #user_location = input("Enter your country of residence : ")


    charity_data=[]
    charity_data=generate_charity_data(charity_data,description,organization_name)
    #charity_data.append(sample_description + " " + user_location)
    charity_data.append(sample_description)
    charity_data = pre_process(charity_data)

    model = SentenceTransformer('bert-base-nli-mean-tokens')
    count_matrix=model.encode(charity_data)
